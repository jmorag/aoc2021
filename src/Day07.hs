...
spec = hspec do
  describe "part 1" do
    it "should take 37 fuel for the crabs to align themselves at position 2" do
      minHorizontalAlignCost constantCost test1 `shouldBe` 37
  describe "part 2" do
  ...
  describe "using math" do
    prop "the optimal alignment position under naive cost should correspond to the median" do
      \(xs :: [Int]) -> minCostP1Smart xs `shouldBe` minCostP1Brute xs
    prop "the optimal alignment position under increasing cost should correspond to the mean" do
      \(xs :: [Int]) -> minCostP2Smart xs `shouldBe` minCostP2Brute xs

constantCost crab pos = abs (crab - pos)

minHorizontalAlignCost _ [] = 0
minHorizontalAlignCost cost crabs =
  minimum $ map (alignCost cost crabs) [minimum crabs .. maximum crabs]

alignCost cost crabs pos = foldMap (\crab -> Sum $ cost crab pos) crabs

increasingCost crab pos = let n = abs (crab - pos) in (n * (n + 1)) `div` 2 -- sum [1..n]

part1 = minCostP1Smart input
part2 = minCostP2Smart input
...
mean = uncurry (/) . go (0, 0)
  where
    go (sum, len) [] = (fromIntegral sum, fromIntegral len)
    go (sum, len) (x : xs) = go (sum + x, len + 1) xs

minCostP1Brute = minHorizontalAlignCost constantCost
minCostP1Smart crabs = alignCost constantCost crabs (median crabs)

minCostP2Brute = minHorizontalAlignCost increasingCost
minCostP2Smart crabs =
  minimum $
    map (alignCost increasingCost crabs) [floor $ mean crabs, ceiling $ mean crabs]
